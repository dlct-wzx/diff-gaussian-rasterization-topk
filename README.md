# Differential Gaussian Rasterization with Contributors

This fork remains API-compatible with the original rasterizer (`gaussian_renderer/__init__.py` is unchanged), but adds **Top-K contributor visualization** via `gaussian_renderer/render_with_contributors.py`. You can drop this repo into any existing 3D Gaussian Splatting project without extra patches—simply rebuild the extension and use the new renderer.
...
- **Visualization helpers** that convert those tensors into color maps for quick inspection.

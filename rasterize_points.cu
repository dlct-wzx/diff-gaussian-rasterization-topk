...
// 新增的RasterizeGaussiansWithTopKContributorsCUDA函数
std::tuple<int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
RasterizeGaussiansWithTopKContributorsCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const bool debug)
{
    ...
    // 创建新的缓冲区，用于存储TopK贡献度信息
    torch::Tensor topk_contributors = torch::zeros({H, W, TOPK_CONTRIBUTORS}, torch::dtype(torch::kInt32).device(device));
    torch::Tensor topk_weights = torch::zeros({H, W, TOPK_CONTRIBUTORS}, torch::dtype(torch::kFloat32).device(device));
	...
	if(P != 0)
	{
		...
		rendered = CudaRasterizer::Rasterizer::forwardWithTopKContributors( \
			...
			out_color.contiguous().data_ptr<float>(), \
			topk_contributors.contiguous().data_ptr<int>(), \
			topk_weights.contiguous().data_ptr<float>(), \
			radii.contiguous().data_ptr<int>(), debug);
	}
    
    // 返回结果，包括TopK贡献度信息
	return std::make_tuple(rendered, out_color, radii, topk_contributors, topk_weights, geomBuffer, binningBuffer, imgBuffer);
}
